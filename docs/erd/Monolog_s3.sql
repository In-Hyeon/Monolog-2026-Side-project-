...
CREATE TABLE IF NOT EXISTS "FriendRequest" (
	...
	"createdAt" TIMESTAMP NOT NULL DEFAULT now(),
	PRIMARY KEY("id"),
	CONSTRAINT "FriendRequest_unique_0" UNIQUE ("requesterId", "addresseeId")
);

COMMENT ON COLUMN "FriendRequest"."id" IS '친구 요청 관계 식별자';
COMMENT ON COLUMN "FriendRequest"."requesterId" IS '요청 보낸 사용자';
COMMENT ON COLUMN "FriendRequest"."addresseeId" IS '요청 받은 사용자';
COMMENT ON COLUMN "FriendRequest"."createdAt" IS '요청 생성 시각';
...
CREATE TABLE IF NOT EXISTS "Friendship" (
	...
	"createdAt" TIMESTAMP NOT NULL DEFAULT now(),
	PRIMARY KEY("id"),
	CONSTRAINT "Friendship_unique_0" UNIQUE ("userId", "friendId")
);

COMMENT ON COLUMN "Friendship"."id" IS '친구 관계 식별자';
COMMENT ON COLUMN "Friendship"."userId" IS 'USER의 ID';
COMMENT ON COLUMN "Friendship"."friendId" IS '친구의 ID';
COMMENT ON COLUMN "Friendship"."createdAt" IS '친구가 된 시각';
